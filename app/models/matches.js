...
module.exports = {
    createRoom: async (body) => {
        const arrays = [];
        const object = { user: body.id };
        arrays.push(object);
        return room = await matchesModel.create({ players: arrays });
    },
    getListRooms: async() => {
        return await matchesModel.find({});
    },
    join_Exit_Finish_Room: async(req) => {
        //tìm phòng theo id
        ...
        if(req.body.option === "addUser") {
        ...
        } else if(req.body.option === "subUser") {
            ...
            room.status = "waiting";
        } else if(req.body.option === "updateResult") {
