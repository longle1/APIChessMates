...
module.exports = {
    validator: (req) => {
        ...
        req.checkBody("gmail", util.format(notify.ERROR_GMAIL_ILLEGAL))
            .matches(/^(([^<>()[\]\.,;:\s@\"]+(\.[^<>()[\]\.,;:\s@\"]+)*)|(\".+\"))@(([^<>()[\]\.,;:\s@\"]+\.)+[^<>()[\]\.,;:\s@\"]{2,})$/i);
        //status active
        req.checkBody("statusActive", util.format(notify.ERROR_NAME_EMPTY, "Status active")).not().isEmpty();
        req.checkBody("statusActive", util.format(notify.ERROR_STATUSACTIVE_ILLEGAL))
            .custom(value => {
                if(value.toLowerCase() === "online" || value.toLowerCase() === "offline") return true;
                return false;
            });
